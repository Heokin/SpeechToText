    ...
    override func viewDidLoad() {
            ...
            if (numberSub == "first") {
                ...
                SwiftyStoreKit.retrieveProductsInfo([second]) { [self] result in
                    if let product = result.retrievedProducts.first {
                        ...
                        } else if second == "com.speakercleaner.1wetr" {
        ...
        if (numberSub == "first") {
            ...
            if self.second == "com.speachtotext.1wetr"{
                self.secondBottomLabel = LabelDual(type: .subscribeBottomPriceActive, wPrice + "\n", local.subscribeWeekly)
                self.secondTopLabel = Label(type: .dayTopActive, local.subscribeFreeTrial)
            } else if self.second == "com.speachtotext.1yetr"{
                self.secondBottomLabel = LabelDual(type: .subscribeBottomPriceActive, yPrice + "\n", local.subscribeYearly)
                self.secondTopLabel = Label(type: .dayTopActive, local.subscribeFreeTrial)
            } else if self.second == "com.speachtotext.1motr"{
                self.secondBottomLabel = LabelDual(type: .subscribeBottomPriceActive, mPrice + "\n", local.subscribeMonthly)
                self.secondTopLabel = Label(type: .dayTopActive, local.subscribeFreeTrial)
            } else if self.second == "com.speachtotext.1we"{
                self.secondBottomLabel = Label(type: .priceBotActive, wPrice)
                self.secondTopLabel = Label(type: .dayTopActive, local.subscribeWeekly)
            } else if self.second == "com.speachtotext.1ye"{
                self.secondBottomLabel = Label(type: .priceBotActive, yPrice)
                self.secondTopLabel = Label(type: .dayTopActive, local.subscribeYearly)
            } else if self.second == "com.speachtotext.1mo"{
                self.secondBottomLabel = Label(type: .priceBotActive, mPrice)
                self.secondTopLabel = Label(type: .dayTopActive, local.subscribeMonthly)
            }
